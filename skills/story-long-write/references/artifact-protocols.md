...
각 산출물의 표준 템플릿과 작성 지침을 정리한 문서다. 에이전트는 2단계에서 3단계로 넘어갈 때 필요에 따라 이 파일을 불러온다.
...
**概念对照（统一叫法，减少混淆）：**
- **플롯 단위** = 하나의 완결된 사건 묶음(1.5만~3만 자 또는 여러 장으로 구성되며, 하나의 갈등이 시작되어 마무리된다).拆书侧就是 `剧情/{剧情单元名}.md`，本书侧就是卷纲里的**剧情单元卡**；理论叫法是「一级结构」。同一粒度，统一叫剧情单元。
- **스토리라인**（`剧情/故事线.md`）= 贯穿多个剧情单元的线（主线/感情线/成长线/夺宝线…），比剧情单元大一层。
...
- 利益安全线：{핵심 자산과 매력이 몰수·양도되거나 대가 없이 노출되지 않도록 경계를 설정한다}
