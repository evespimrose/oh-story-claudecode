...
姓名：
性别：
캐릭터 포지션: (주인공이 이야기에서 맡는 기능을 한 문장으로 분명히 쓴다)
...
核心目标：（全书终点想达成什么）
핵심 동기: (반드시 이루어야 하는 이유 — 감정적 동기이지 이성적 동기가 아니다)
치명적 약점: (캐릭터가 실수하게 만드는 성격적 결함이지 능력의 결함이 아니다)
말버릇·상징 동작: (독자가 즉시 알아보게 하는 표지)
```

### 设计要点

- 동기는 감정적 차원에 있어야 한다 — 「어머니의 복수」가 「최강이 되겠다」보다 우선한다
- 약점은 핵심 장면에서 주인공의 실수를 유발해야 한다. 그렇지 않으면 약점이 아니다
- 外貌和口头禅是读者记忆锚点，必须写
...
- 모든 조연은 명확한 기능을 가져야 한다——기능이 없는 캐릭터는 등장시키지 않는다
- 配角退场要主动规划，不能写着写着忘了
- 同一场景配角不超过 3 个有台词
- 이름은 설정한 시대를 따른다、地域、文化走；同人或既有世界观里，龙套和新配角也要顺原著对应地域的命名风格，别混语系、别塞与设定不符的现代化或政治化名字
