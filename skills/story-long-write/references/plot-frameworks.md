...
> **지시 어조:** 직접적이고 구체적이며 실행 가능해야 한다. ‘고려할 수 있다’가 아니라 ‘반드시 해야 한다’고 쓴다. 모든 항목은 이론 토론이 아니라 바로 적용할 수 있는 집필 작업이다.
>
> **원문 앵커:** 장르명·고정 서사 패턴·고전 사례는 원문 앵커로 보존하고, 일반 지침과 설명은 한국어로 현지화한다.

---

## 의사결정 라우팅 표

현재 집필 장면에 맞는 절을 찾는다. 처음부터 끝까지 읽지 말고 필요한 부분만 사용한다.
...
| 장르 유형 | 핵심 순환 패턴 | 주 프레임 | 보조 프레임 |
|---------|------------|---------|---------|
| 승급물(현환/선협/도시 이능) | 보물 쟁탈+비무+연애 요소 3축 교대 | 현환 프레임 분해 | RPG 보상 설계·세력 패 카드법 |
| 시스템물/무한류 | 임무→보상→교환→새 임무 | 핵심 소재 순환법 | RPG 보상 설계 |
| 후회·재결합물 | 상처→후회→만회→반복된 밀당 | 이중선과 정보 격차 | 구조·인물 관계·서스펜스 기법 |
| 회귀 복수 | 전생 기억→설계→반격→승급 | 들어옴만 있고 나감은 없는 원칙 | 9개 메인선 클리셰법 |
| 도시 사이다물 | 약한 척→강자 제압→충격→지도 전환 | 클리셰 템플릿 반복법 | 세력 패 카드법·사이다 5단계법 |
| 농장·기반 건설물 | 준비→건설→수확→확장 | RPG 보상 설계 | 핵심 소재 순환법 |
| 은둔 고수물 | 실력 은닉→저평가→압도→계속 은닉 | 이중선 교대 프레임 | 정보 격차 기법 |
| 달달 일상물 | 작은 충돌→해결→친밀감→새 충돌 | 구조화 작품 분해법(브리지 4장 구조) | 완충 지대 개념 |
| 미스터리·추리 | 단서→추론→반전→새 수수께끼 | 이야기 본질과 6막 구조 | 서스펜스 기법·복선 전개 |
| 열혈 전투 | 승급→도전→실패→폭발→압도 | 사이다 5단계법+점진적 위압 연출 | 들어옴만 있고 나감은 없는 원칙 |
...
- 메인선: 주인공이 행동하는 과정에서 감정을 만든다.
- 보조선: 조연이 주인공의 신분·태도를 파헤치는 과정에서 감정을 만든다.
- 주인공의 숨은 신분·인맥·배경을 분명히 쓰고 완충 지대를 설계한다.
- 조연이 계속 발견하고 계속 충격받는 흐름은 훌륭한 감정선이 된다.

### 정보 격차의 활용

- 정보 격차는 병렬 연결의 핵심 도구다.
- 주인공과 독자는 알지만 조연은 모름 -> 조연이 발견할 때 충격이 생긴다.
- 정보 격차로 여러 층의 사이다를 만들 수 있다.
- 시점을 전환할 때 정보 격차를 훅으로 써서 독자의 관심을 다시 끌어올린다.
...
주인공과 조연 사이에는 이익·사건·감정 때문에 거리가 생긴다. 그 거리를 계속 공략하면서 사이다가 만들어진다. 완충 지대를 설계하려면 주인공의 배경 이야기를 분명히 써야 한다.
...
- 机位2：目标视角（主角做了什么反应）
- 不要同时搞太多机位
- 정보 덩어리의 핵심을 바탕으로 머릿속에서 장면을 촬영한다
