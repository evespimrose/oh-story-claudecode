...
> **문체**: 지시형 — 모든 원칙은 반드시 지켜야 하는 강행 규칙이며, 어기면 명확한 결과가 따른다.
...
> 개요를 구상할 때는 단계별로 진행하고, 각 단계마다 해당 도구로 대조한다.
...
| 정의 | 해당 장르만의 핵심 매력 |
| 作用 | 筛选读者、划定边界、产生氛围感 |
| 추출법 | 같은 장르의 인기작에서 핵심 매력과 핵심 감정을 확인한다 |

**핵심 원칙: 장르의 핵심에서 벗어나는 것은 독자를 끌어들이는 가장 큰 무기를 버리는 일이며, 독서 지속률이 곧바로 떨어진다.**
...
| 维度 | 说明 |
|------|------|
| 정의 | 一种核心情绪体验(扮猪吃虎/弥补遗憾/莫欺少年穷等) |
| 作用 | 筛选读者、提升戏剧性 |

핵심 감정은 쾌감 포인트와 같지 않다表现形式。装逼打脸只是形式，核心梗是完整的情绪链条(期待→满足)。
...
각 장에는 다음 세 가지 중 하나가 반드시 있어야 한다：①拥有核心梗相关的期待点 ②拥有核心梗相关的爽点 ③介于期待点和爽点之间。

**기대 단절의 판단**：某章既无期待点也无爽点，前一爽点已结束，本身没拉起新期待→读者觉得书失去了"灵魂"开始弃书。
...
- [ ] **三层递进齐全**：主题(立意)→题材核心(吸引力)→核心情绪(体验链条)三层都有정의
