...
1. [단편 vs 장편: 핵심 차이](#단편-vs-장편-핵심-차이)
2. [분해 접근법](#분해-접근법)
3. [5단계 분해 절차](#5단계-분해-절차)
4. [플롯 노드 추출 규칙](#플롯-노드-추출-규칙)
5. [집필 기법 분석](#집필-기법-분석)
6. [감정선 분석](#감정선-분석)
7. [리듬 분석](#리듬-분석)
8. [인물 추출 규칙](#인물-추출-규칙)
9. [공감 분석](#공감-분석)
10. [구조 유형 빠른 참조](#구조-유형-빠른-참조)
11. [품질 기준](#품질-기준)
12. [references 협업 관계](#references-협업-관계)
...
| 차원 | 장편 | 단편 |
|------|------|------|
| 분량 | 수십만 자·수백 장 | 5,000~15,000자·6~12절 |
| 곡선 | 다선 교차·다권 구조 | 단일 곡선·압축된 3막 |
| 인물 | 군상·등급 체계 | 정제된 3~5명·기능 고밀도 |
| 세계관 | 복잡한 설정·힘의 체계 | 현실 중심·특수 설정 최소화 |
| 핵심 구동 | 사이다 순환·승급감 | 폭발점 설계·반전 충격 |
| 독자 유지 | 연재 추독 구동 | 한 번에 완독 |
| 분석 중심 | 구조 완결성·복선 관리 | 감정 설계·집필 기법·리듬 |

**핵심: 단편은 공감과 폭발점으로 움직인다. 작품 분해는 어떤 이야기 핵을 쓰고 어떻게 포석하며 어디에서 터뜨리는지 보는 일이다.**
...
| 원칙 | 설명 |
|------|------|
| 이야기 핵 구동 | 먼저 이야기의 핵심 소재를 한 문장으로 찾고, 모든 분석을 그 핵을 중심으로 전개한다. 이야기 핵이 바뀌면 분석 방향도 달라진다. |
| 독자 시점 우선 | 모든 단계는 독자 경험에서 출발한다. 독자가 무엇을 보고 느끼며 왜 관심을 두는지 분석하고, 창작자 의도는 추측하지 않는다. |
| 재사용성 지향 | 모든 분석점은 옮겨 쓸 수 있는 구조·기능 위치·위험 임계값을 출력해야 한다. 순수 감상형 분석은 부적합하다. |
| 폭발점 중심 | 단편 분해의 핵심은 무엇이 독자의 감정을 터뜨리고 공유하고 싶게 하는지 찾는 것이다. 폭발점의 포석·해소 메커니즘을 분해한다. |
| 공감이 전파를 결정 | 단편이 흥하는 핵심은 공감이다. 문장력만으로 전파가 결정되지는 않으며 공감 층위가 많을수록 전파력도 강해진다. |
