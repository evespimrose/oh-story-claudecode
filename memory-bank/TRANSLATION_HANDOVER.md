...
현재 기준 브랜치는 `main`이며, 로컬 `HEAD`와 `origin/main`은 다음 커밋에서 동기화되어 있다.
...
Batch 26~28에서 다음 스크립트의 일반 주석, JSDoc, 사용자 표시 로그와 오류 메시지를 한국어로 현지화했다.
...
Batch 28의 네 JavaScript 파일은 `node --check`를 통과했고, 전체 변경은 `git diff --check`를 통과했다. `demo/` 본문은 Batch 28에서 수정하지 않았다.
